...
import { createChecklistBlock, renumerarChecklist } from './blocks/checklist.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const checklistContainer = document.getElementById('aitg-checklist-container');
    ...
    const refreshChecklist = () => {
        renumerarChecklist(checklistContainer);
        toggleEmptyState('aitg-checklist-container', 'aitg-checklist-empty', '.aitg-checklist-block');
    };
    ...
    const addChecklist = (data = {}) => {
        const block = createChecklistBlock(checklistContainer, (node) => {
            node.remove();
            refreshChecklist();
        }, data);
    ...
    btnAddChecklist?.addEventListener('click', () => addChecklist());
    ...
    (config.checklist || []).forEach((item) => addChecklist(item));
